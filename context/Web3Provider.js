...
export const Web3Provider = ({ children }) => {
      ...
      try {
        ...
        if (address && info.tokenAddress) {
          ...
          setTokenBalances((prev) => ({
            ...prev,
            usertbcBlanace: ethers.utils.formatUnits(
              userTokenBalance,
              tokenDecimals
            ),
  ...
  const buyToken = async (ethAmount) => {
    ...
    try {
      ...
      if (receipt.status === 1) {
        // Calculate tokens received based on fixed price of 0.01 POL per token
        const tokenPrice = PER_TOKEN_USD_PRICE; // Price per token in POL
